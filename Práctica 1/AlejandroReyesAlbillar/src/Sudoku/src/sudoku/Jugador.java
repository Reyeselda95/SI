...
public class Jugador {
    ...
    /**
     * Comprueba que las casillas cargadas del jugador corresponden al tablero actual
     * @param tablero
     * @param casillas
     * @return 
     */
    public boolean actual(Tablero tablero, Casilla [] casillas){
        int pos=0;
        for(int i=0;i<9;i++){
            for(int j=0;j<9;j++){
                if(tablero.getCasilla(i, j)!= casillas[pos].getValor()){
                    return false;
                }
                pos++;
            }
    ...
    public boolean ejecutarBC(Tablero tablero) {
        ...
        if (!buscadoAC3 || !actual(tablero, casillas)) {
